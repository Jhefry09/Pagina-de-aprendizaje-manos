...
const DashboardPage: React.FC = () => {
  ...
  // Obtener y persistir la URL base
  useEffect(() => {
    const saved = localStorage.getItem('progress_base_url');
    if (saved) setBaseUrl(saved);
  }, []);
  useEffect(() => {
    localStorage.setItem('progress_base_url', baseUrl);
  }, [baseUrl]);
  ...
  const endpoints = useMemo(() => ({
    progresoUsuario: (id: number) => `${baseUrl}/api/progreso/usuario/${id}`,
    letrasCompletadas: (id: number) => `${baseUrl}/api/progreso/letras/${id}`,
    completar: () => `${baseUrl}/api/progreso/completar`,
    verificar: (usuario: number, vocal: number) => `${baseUrl}/api/progreso/verificar/${usuario}/${vocal}`,
    reiniciar: (id: number) => `${baseUrl}/api/progreso/reiniciar/${id}`,
  }), [baseUrl]);
    ...
    <div className="min-h-screen">
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-6 sm:py-8 w-full">
        ...
        <div className="mb-8 sm:mb-10">
          <SectionCard title="Configuración del Backend" icon="⚙️" accent="from-amber-400 to-yellow-500">
            <Field label="URL del Backend">
              <Input type="text" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} />
            </Field>
            <p className="text-xs text-gray-500">Asegúrate de que tu backend esté ejecutándose en esta URL.</p>
          </SectionCard>
        </div>
